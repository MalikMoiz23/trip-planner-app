...
import 'package:trip_planner/shared/widgets/ambient.dart';
...
/// gap if it does not.
///
/// Takes plain values rather than reading the planner out of the tree. It is
/// used from two different features, and a shared widget that reaches into one
/// feature's controller quietly welds the two together — the layering test
...
class BudgetPanel extends StatelessWidget {
  ...
  /// Null until the route has been measured.
  final BudgetAdvice? advice;
  final ValueChanged<BudgetLever> onApplyLever;
        ...
        if (hasBudget) ...[
          ...
          if (advice == null)
            ...
            if (!compact && !advice!.fits) ...[
              ...
              _Levers(advice: advice!, onApply: onApplyLever),
            ],
          ],
        ],
      ],
...
class _Levers extends StatelessWidget {
  const _Levers({required this.advice, required this.onApply});

  final BudgetAdvice advice;
  final ValueChanged<BudgetLever> onApply;
